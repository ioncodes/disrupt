class Disassembler {
  constructor(arch, _mode, syntax) {
    this.capstone = require('capstone');
    var architecture = this.getArchitecture(arch);
    var mode = this.getMode(_mode);
    if(syntax) {
      var syntax = this.getSyntax(syntax);
      this.cs = new this.capstone.Cs(architecture, mode, syntax);
    } else {
  ...
  getSyntax(syntax) {
    switch (syntax.toUpperCase()) {
      case 'INTEL':
        return this.capstone.SYNTAX_INTEL;
        break;
      case 'ATT':
        return this.capstone.SYNTAX_ATT;
        break;
      case 'NOREGNAME':
        return this.capstone.SYNTAX_NOREGNAME;
        break;
      default:
